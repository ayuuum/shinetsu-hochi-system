...
| **WARNING** | 期限30日前 | ダッシュボードのみ | 🟡 黄 |
| **URGENT** | 期限14日前 | ダッシュボードのみ | 🟠 橙 |
| **CRITICAL** | 期限切れ | ダッシュボードのみ | 🔴 赤 |
...
```mermaid
flowchart TD
    A[ダッシュボード/一覧表示] --> B{全期限付きレコード<br/>を確認}
    B --> C{残日数を算出}
    C -->|60日以内| D[INFO: 画面表示]
    C -->|30日以内| E[WARNING: 画面表示]
    C -->|14日以内| F[URGENT: 画面表示]
    C -->|0日以下| G[CRITICAL: 画面表示]
    
    D & E & F & G --> H[ダッシュボードに反映]
```

> メール通知は運用対象外。期限切れ・期限間近の確認はダッシュボードと各一覧画面で行う。
...
作業員が「不適正」を選択
  ...
  → 記録保存
  → 画面内アラート: ダッシュボードと一覧で赤アラート表示（メール通知はしない）
...
> 従来の REST API エンドポイントは不要。API Route はエクスポート機能を中心に最小限とする。
...
| メソッド | パス | 説明 |
|---|---|---|
| GET | `/api/export/employees` | 社員一覧CSVエクスポート |
| GET | `/api/export/qualifications` | 資格一覧CSVエクスポート |
| GET | `/api/export/vehicles` | 車両・備品CSVエクスポート |
| GET | `/api/export/projects` | 施工実績CSVエクスポート |
| GET | `/api/export/health-checks` | 健康診断CSVエクスポート |
| GET | `/api/export/alcohol-checks` | アルコールチェックCSVエクスポート |
...
> [!NOTE]
> フルコード型での開発が確定しました。既存Excelの社員台帳はCSVに変換し、社員CSVインポートで移行します。
...
gantt
    ...
    section Day 3 アラート
    講習期限算出ロジック + 画面内アラート  :d3, 2026-03-12, 1d
...
## 9.5 社員CSVデータインポート画面（P1）

> [!NOTE]
> デプロイ後の最初の操作。既存ExcelをCSVに変換し、社員データの一括登録を行う。Excelファイルの直接取込は対象外。

**フロー:**
1. テンプレートCSVをダウンロード
2. 既存Excelから必要列を転記し、CSV形式で保存
3. ファイルアップロード（drag & drop + ファイル選択）
4. カラムマッピング（「社員番号 → employee_number」等を自動対応）
5. バリデーション結果表示:
   - 成功行: 緑 ✓
   - エラー行: 赤 ✗ + 行番号 + エラー理由（例: 「3行目: 生年月日が不正」）
   - サマリー: 「45行中 42行OK、3行エラー」
6. 「エラーなし行のみ登録」or「修正して再アップロード」の選択
7. インポート実行 + 完了サマリー: 「42名の社員を登録しました」

**社員CSV仕様:**
- 必須列: 社員番号、氏名、フリガナ、生年月日、入社日、拠点
- 任意列: 性別、電話番号、メール、住所、雇用形態、職種、役職、雇用保険番号、保険証番号、厚生年金番号
- 日付形式: `YYYY-MM-DD` または `YYYY/M/D`
- 重複判定: 社員番号（CSV内・既存DB）
- 上限: 1回あたり1,500行

**エクスポートテンプレート:**
> 施工実績書の正式PDF/Excelテンプレート対応は後続対応。既存の官公庁提出用フォーマット取得後にSection 3.4を更新する。
...
| 除外項目 | 理由 |
|---|---|
| 技術者向けマイページ/ランディング | `/today` と `/me` で実装済み。追加改善は現場運用後に判断 |
