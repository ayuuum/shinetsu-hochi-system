...
BEGIN {
	...
	LINES=0;
	printf("INSERT INTO m_orderingpatries VALUES\n") > "m_orderingpatries.sql";
}
{ 
	if(NF<fields)
	{
...
function writesql(arr)
{
	...
	if(LINES>0)printf(",") > "m_orderingpatries.sql";
	#################################### printf (0,01,02,03,04,05,06,07,08,09,10,11)
	printf("(0,%d,%d,%s,%s,%s,%s,%s,%s,%d,%s,%s)\n", array[1], array[2], array[3], array[4], array[5], array[6], array[7], array[8], array[9],now,now) > "m_orderingpatries.sql";
	LINES++;
}
END {
	printf(";") > "m_orderingpatries.sql";
	print "["LINES"] records converted!";
	close("m_orderingpatries.sql");
}
