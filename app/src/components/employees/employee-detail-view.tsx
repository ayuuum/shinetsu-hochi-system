...
const LIST_HREF: Record<EmployeeListContext, string> = {
    employees: "/employees",
    partners: "/partners",
};

const LIST_LABEL: Record<EmployeeListContext, string> = {
    employees: "社員一覧へ戻る",
    partners: "協力会社台帳へ戻る",
};

function AccessDeniedView({ listContext }: { listContext: EmployeeListContext }) {
    return (
        <div className="space-y-5 animate-in fade-in duration-200">
            <Button variant="ghost" size="sm" className="w-fit rounded-full px-2.5 text-muted-foreground" render={<Link href={LIST_HREF[listContext]} />}>
                <ArrowLeft className="mr-1.5 h-4 w-4" />
                {LIST_LABEL[listContext]}
            </Button>
            <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
                <Lock className="h-10 w-10 text-muted-foreground/50" />
                <div className="space-y-2">
                    <h1 className="text-xl font-semibold tracking-tight">この情報は閲覧できません</h1>
...
export async function EmployeeDetailView({
    ...
    id: string;
    tab?: string;
    listContext: EmployeeListContext;
}) {
    ...
    if (employeeResult.error) {
        ...
        if (!usingServiceRole && !isAdminOrHr) {
            return <AccessDeniedView listContext={listContext} />;
        }
        notFound();
    }
