                    ...
                    {(!employee.employee_life_insurances?.length && !employee.employee_damage_insurances?.length) ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {employee.employee_life_insurances?.map((insurance) => (
                                <Card key={insurance.id} className="shadow-sm border-border/50 relative overflow-hidden group">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            {isAdminOrHr && (
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <Button variant="ghost" size="sm" onClick={() => setDeletingLifeInsuranceId(insurance.id)} className="h-8 w-8 p-0 text-destructive hover:text-destructive hover:bg-destructive/10"><Trash2 className="h-4 w-4" /></Button>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    <CardContent className="space-y-0 pl-6">
                                        <DetailItem label="加入日" value={insurance.start_date || "-"} />
                                        <DetailItem label="満期日" value={insurance.maturity_date || "-"} />
                                        <div className="grid grid-cols-3 border-b border-border/50 py-3 last:border-0 bg-primary/5 -mx-6 px-6 relative mt-2">
                                            ...
                                            <span className="col-span-2 font-bold text-sm text-primary flex items-center">{insurance.peak_date || "未設定"}</span>
                                        </div>
                                        {insurance.notes && (
                            ...
                            {employee.employee_damage_insurances?.map((insurance) => (
                                <Card key={insurance.id} className="shadow-sm border-border/50 relative overflow-hidden group">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            {isAdminOrHr && (
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <Button variant="ghost" size="sm" onClick={() => setDeletingDamageInsuranceId(insurance.id)} className="h-8 w-8 p-0 text-destructive hover:text-destructive hover:bg-destructive/10"><Trash2 className="h-4 w-4" /></Button>
                                                </div>
                                            )}
                                        </div>
                                        <CardTitle className="text-xl mt-2">{insurance.insurance_name}</CardTitle>
                                    ...
                                    <CardContent className="space-y-0 pl-6">
                                        <DetailItem label="更新日(更改日)" value={insurance.renewal_date || "-"} />
                            ...
                            {employee.employee_qualifications.map((qualification) => (
                                <Card key={qualification.id} className="shadow-sm">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                {isAdminOrHr && (
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        aria-label={`${qualification.qualification_master?.name || "資格"}を削除`}
                                                        className="h-6 w-6 p-0 text-destructive hover:text-destructive"
                                                        onClick={() => setDeletingQualId(qualification.id)}
                                                    >
                                    ...
                                    <CardContent className="text-sm space-y-1">
                                        ...
                                        <div className="flex justify-between"><span className="text-muted-foreground">取得日</span><span>{qualification.acquired_date || "-"}</span></div>
                                        <div className="flex justify-between font-bold">
                                            <span className="text-muted-foreground">有効期限</span>
                                            <span className={qualification.expiry_date && new Date(qualification.expiry_date) < new Date() ? "text-destructive" : ""}>
                                                {qualification.expiry_date || "期限なし"}
                                            </span>
                                        </div>
                                        ...
                                        <div key={record.id} className="flex flex-col border-b last:border-0 pb-4 last:pb-0">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="text-xs text-muted-foreground">{record.construction_date}</span>
                                                    {isAdminOrHr && (
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            aria-label={`${record.construction_name}を削除`}
                                                            className="h-6 w-6 p-0 text-destructive hover:text-destructive"
                                                            onClick={() => setDeletingConstructionId(record.id)}
                                                        >
                                                            <Trash2 className="h-3.5 w-3.5" />
                                                        </Button>
                                                    )}
                                ...
                                <Card key={family.id} className="shadow-sm">
                                    <CardContent className="p-4 flex justify-between items-center">
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-1">{family.birth_date || "不明"} 生まれ</p>
                                        </div>
                                ...
                                <div className="space-y-6">
                                    {employee.health_checks.map((healthCheck) => (
                                        <div key={healthCheck.id} className="relative pl-6 border-l-2 border-primary/20 last:border-l-transparent">
                                            ...
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <span className="text-xs font-bold text-primary">{healthCheck.check_date} 実施</span>
